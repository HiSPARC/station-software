...
# severity support is not implemented yet, but the constants are here so the interface is ready
SEVERITY_NORMAL = 1
SEVERITY_CRITICAL = 2

# "private" function
# extracts current application name by taking the directory of the initially invoked .py file.
def getCurrentAppName():
	mo = re.search('\\\\([^\\\\]*)$', sys.path[0])
	return mo.group(1)	

# sends a message to screen, to a file, or both, depending on the logMode (see below).
# the optional severity parameter is not implemented yet, but you are still recommended
# to pass SEVERITY_CRITICAL in case you are logging an error that cannot be recovered from
