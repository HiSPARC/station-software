...
class Updater:
	...
	timeBetweenChecks = 0 #The amount of time in seconds between two consecutive update checks
	...
	def calculateInitialDelay(self):
		if self.checkerInitialDelay == 1:
			...
			if ((today_at_starttime < now) & (now < today_at_stoptime)):
				...
				tomorrow_random_moment = random.randint(tomorrow_at_four, tomorrow_at_noon)
	...
	def performContinuousCheck(self):
		while (True):
			self.scheduler.enter(self.timeBetweenChecks, 1, self.checker.checkForUpdates, '')
			self.scheduler.run()
	
	def __init__(self):
		self.config.read([CONFIG_INI, PERSISTENT_INI])
		self.timeBetweenChecks = int(self.config.get('Update', 'IntervalBetweenChecks'))
		self.timeStartCheckInterval = int(self.config.get('Update', 'CheckerIntervalStartTime'))
		self.timeStopCheckInterval = int(self.config.get('Update', 'CheckerIntervalStopTime'))
		self.checkerInitialDelay = int(self.config.get('Update', 'CheckerInitialDelay'))
...
try:
	...
	log("Updating failed due to %s, restart the checker!" % str(sys.exc_info()[1]), severity=SEVERITY_CRITICAL)
	exit
