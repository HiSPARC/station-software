...
class Checker:
	...
	def requestCheckFromServer(self):
		...
		intervalBetweenChecks = self.config.get('Update', 'IntervalBetweenChecks')
		
		params = urlencode(
			{
				'admin_version': currentAdmin,
				'user_version': currentUser,
				'station_id': stationname,
			}
		)
		
		#urlopen('%s?%s' % (server, params))
	...
	def parseAnswerServer(self, updateInfo):
		updateDict = parse_qs(updateInfo, strict_parsing=True) #updateDict has: mustUpdate, urlUser, newVersionUser, urlAdmin, newVersionAdmin
		...
		if (mustUpdate & self.UPDATE_ADMIN_MODE):
			...
			#print 'Administrator update is available called: %s' % adminFile
			
			if DISPLAY_GUI_MESSAGES and not(checkFiles.checkIfAdmin()):
				root = Tk()
				root.title('HiSparc')
				Message(root, anchor='s', text="Update is available requiring administrator rights!\nPlease ask your administrator to reboot and install it!").pack(padx=150, pady=100)
				root.mainloop()
	...
	def checkForUpdates(self):
		try:
			...
			#print 'Could not reach the server to check for updates: : %s' % str(sys.exc_info()[1])
			log('Could not reach the server to check for updates: : %s' % str(sys.exc_info()[1]), severity=SEVERITY_CRITICAL)
			return
		try:
			updates = self.parseAnswerServer(updateInfo)
			return updates
		except:
			#print 'Could not parse the answer of the server correctly: %s' % str(sys.exc_info()[1])
			log('Could not parse the answer of the server correctly: %s' % str(sys.exc_info()[1]), severity=SEVERITY_CRITICAL)
			return
